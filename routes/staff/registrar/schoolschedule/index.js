...
////////////////// checking school schedule module //////////////////
function registrarmodule(req, res, next) {
  let sql =
    "SELECT school_sched_pic FROM registrar_module WHERE school_sched_pic=? AND id=?";
  db.query(
    sql,
    ["staffmodule7", res.locals.currentid],
    (err, results, fields) => {
      if (results.length > 0) {
        next();
      } else {
...
function getfilename(req, res, next) {
  let sql = "SELECT filename FROM school_schedule_tbl";
...
function saveschedule(req, res, next) {
  ...
  let sql = "UPDATE school_schedule_tbl SET ?";
