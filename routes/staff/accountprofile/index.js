...
function displayallinfo(req, res, next) {
  let sql =
    "SELECT staff_tbl.id, staff_tbl.lastname, staff_tbl.firstname, " +
    "staff_tbl.username, staff_tbl.password," +
    "staff_info.picture, staff_info.birthdate, " +
    "staff_info.phone, staff_info.email, staff_info.address " +
    "FROM staff_tbl INNER JOIN staff_info " +
...
function updateadmininfo(req, res, next) {
  ...
  let sql = "UPDATE staff_info SET ? WHERE id=?";
