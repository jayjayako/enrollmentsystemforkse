...
router.use((req, res, next) => {
  try {
    let sql = "SELECT id, lastname, firstname FROM admin_tbl WHERE id = ?";
...
router.use((req, res, next) => {
  try {
    ...
    db.query(sql, [res.locals.currentid], (err, results, fields) => {
      if (results.length > 0) {
        res.locals.adminmodule = results;
      }
      next();
    });
  } catch (error) {
...
router.use((req, res, next) => {
  try {
    let sql =
      "SELECT COUNT(*) AS totalregistrar FROM staff_info WHERE position = ?";
    db.query(sql, ["Registrar"], (err, results, fields) => {
      if (results.length > 0) {
        res.locals.registrarstats = results[0].totalregistrar;
      }
      next();
    });
  } catch (error) {
...
router.use((req, res, next) => {
  try {
    let sql =
      "SELECT COUNT(*) AS totalaccounting FROM staff_info WHERE position = ?";
    db.query(sql, ["Accounting"], (err, results, fields) => {
      if (results.length > 0) {
        res.locals.accountingstats = results[0].totalaccounting;
      }
      next();
    });
  } catch (error) {
...
router.use((req, res, next) => {
  try {
    let sql =
      "SELECT COUNT(*) AS totalcashier FROM staff_info WHERE position = ?";
    db.query(sql, ["Cashier"], (err, results, fields) => {
      if (results.length > 0) {
        res.locals.cashierstats = results[0].totalcashier;
      }
      next();
    });
  } catch (error) {
...
router.use((req, res, next) => {
  try {
    let sql =
      "SELECT COUNT(*) AS totaladmindept FROM staff_info WHERE position = ?";
    db.query(sql, ["Admin Department"], (err, results, fields) => {
      if (results.length > 0) {
        res.locals.admindeptstats = results[0].totaladmindept;
      }
      next();
    });
  } catch (error) {
