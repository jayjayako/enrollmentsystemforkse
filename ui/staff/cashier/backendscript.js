...
function cashierfunct(options) {
  var myresult;
  var myprofilepic;

  async function checkcashiermodule(position) {
    try {
      ...
      let response = await fetch("/api/staff/cashier/checkmodule", {
        method: "GET",
      });
      let myobj = await response.json();
      if (myobj[0].verify_payment == "invalid") {
        document.getElementById("verifypaymentcard").style.display = "none";
      }
      if (myobj[0].edit_payment_method == "invalid") {
        document.getElementById("editpaymentmethodcard").style.display = "none";
      }
      if (myobj[0].view_stdnt_blnce == "invalid") {
        document.getElementById("viewstudentbalancecard").style.display =
          "none";
      }
    } catch (error) {
      console.log("loading modules....");
      setTimeout(checkcashiermodule, 1000, position);
    }
  }
  ...
  async function checkuser() {
    try {
      ...
      if (myresult != "invalid") {
        ...
        setTimeout(checkcashiermodule, 500, myobj[0].position);
      } else {
