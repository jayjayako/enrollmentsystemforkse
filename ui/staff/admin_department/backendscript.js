var admindeptsuperidglobalvar, globalvarposition;
async function displaystudentpersonalinfo() {
  let response = await fetch(
    "/api/staff/admindepartment/displaystudentpersonalinfo?superid=" +
...
async function displayclickabletableadmindepartment(superid) {
  ...
  let response = await fetch(
    "/api/staff/admindepartment/displayspecificdata?superid=" + superid,
    {
      method: "GET",
    }
  );
...
async function checkadmindepartmentmodule(position) {
  ...
  document.getElementById("admindepartmentsidenavid").style.display = "none";
  ...
  if (position == "Cashier") {
    document.getElementById("cashiersidenavid").style.display = "inline-block";
  }
...
function admindepartmentfunc(options) {
  ...
  async function checkuser() {
    try {
      ...
      if (myresult != "invalid") {
        ...
        setTimeout(checkadmindepartmentmodule, 500, myobj[0].position);
      } else {
  ...
  async function displayschoollevel() {
    try {
      let response = await fetch(
        "/api/staff/admindepartment/displayschoollevel",
        {
          method: "GET",
        }
      );
  ...
  async function displayyearlevel() {
    try {
      ...
      let response = await fetch(
        "/api/staff/admindepartment/displayyearlevel?schoollevel=" +
          schoollevel,
        {
          method: "GET",
        }
  ...
  async function displaysection() {
    try {
      ...
      let response = await fetch(
        "/api/staff/admindepartment/displaysection?schoollevel=" +
          schoollevel +
          "&yearlevel=" +
          yearlevel,
        {
  ...
  async function displaytable() {
    ...
    let response = await fetch(
      "/api/staff/admindepartment/displaytable?schoollevel=" +
        schoollevel +
        "&yearlevel=" +
        yearlevel +
        "&section=" +
  ...
  async function enlist() {
    let response = await fetch(
      "/api/staff/admindepartment/enlist?superid=" + admindeptsuperidglobalvar,
      {
        method: "POST",
      }
    );
  ...
  async function displaytableenrolled() {
    ...
    let response = await fetch(
      "/api/staff/admindepartment/displayenrolledtable?schoollevel=" +
        schoollevel +
        "&yearlevel=" +
        yearlevel +
        "&section=" +
