...
function accountprofilefunct(options) {
  ...
  async function checkuser() {
    try {
      let response = await fetch("/api/staff/accountprofile/getaccount", {
        method: "GET",
      });
      ...
      if (myresult != "invalid") {
        ...
        staffidglobalvar = myobj[0].id;
        accountprofilefunct("activitylogstaff");
      } else {
  ...
  async function editprofilefunc() {
    try {
      ...
      let response = await fetch("/api/staff/accountprofile/editmyprofile", {
        method: "POST",
        body: formData,
      });
      let myobj = await response.json();
  ...
  async function activitylogstaff() {
    try {
      let response = await fetch(
        "/api/staff/dashboard/displaystaffactivitylog?id=" + staffidglobalvar,
        {
          method: "GET",
        }
      );
