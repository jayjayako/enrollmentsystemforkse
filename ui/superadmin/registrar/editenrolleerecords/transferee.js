//////////////////// THIS IS FOR TRANSFEREE STUDENT BACKEND /////////////////
var transfereestudentglobalvar; // this is a global variable

async function editenrolleerecordstransfereestudentdisplaystudentinfo(
  studentid
) {
  let response = await fetch(
    "/api/superadmin/registrar/admission/displaytransfereestudentfiles?studentid=" +
      studentid,
    {
      method: "GET",
    }
  ...
  if (myresult[0].id != "invalid") {
    ...
    document.getElementById("transfereestudentresult2").innerHTML = text;
  }

  let response2 = await fetch(
    "/api/superadmin/registrar/admission/displayspecifictransfereestudent?studentid=" +
      studentid,
    {
      method: "GET",
    }
  ...
  if (myresult2[0].id != "invalid") {
    ...
    ///////////////////////////////////////////////////////////////
    document.getElementById("transfereestudentwholename").value =
...
function editenrolleerecordstransfereestudent(options) {
  /////////////////////// this is for displaying new student ////////////////////////
  async function displaystudenttable() {
    let response = await fetch(
      "/api/superadmin/registrar/admission/displaytransfereestudent",
      {
        method: "GET",
      }
    );
    ...
    if (myresult[0].id != "invalid") {
      for (var x in myresult) {
        text +=
          "<tr onclick=" +
          '"' +
          "editenrolleerecordstransfereestudentdisplaystudentinfo('" +
          myresult[x].id +
          "')" +
          '"' +
          "><td>" +
      ...
      document.getElementById("transfereestudentresult1").innerHTML = text;
      document.getElementById("transfereestudentresult2").innerHTML = text2;
    } else {
      document.getElementById("transfereestudentresult1").innerHTML = text;
      document.getElementById("transfereestudentresult2").innerHTML = text2;
    }
  ...
  async function approvestudent() {
    var studentid = transfereestudentglobalvar;

    let response = await fetch(
      "/api/superadmin/registrar/admission/approvetransfereestudent?studentid=" +
        studentid,
      {
        method: "GET",
      }
    ...
    if (myresult[0].id == "success") {
      loadalert();
      document.getElementById("transfereestudentresult1").innerHTML = "";
      document.getElementById("transfereestudentresult2").innerHTML = "";
