/////////////////////////////// THIS IS FOR NEW STUDENT BACKEND /////////////////////////
var newstudentglobalvar; // this is a global variable

async function editenrolleerecordsnewstudentdisplaystudentinfo(studentid) {
  let response = await fetch(
    "/api/superadmin/registrar/admission/displaynewstudentfiles?studentid=" +
      studentid,
    {
      method: "GET",
    }
  ...
  if (myresult[0].id != "invalid") {
    ...
    document.getElementById("newstudentresult2").innerHTML = text;
  }

  let response2 = await fetch(
    "/api/superadmin/registrar/admission/displayspecificnewstudent?studentid=" +
      studentid,
    {
      method: "GET",
    }
  ...
  if (myresult2[0].id != "invalid") {
    ...
    ///////////////////////////////////////////////////////////////
    document.getElementById("newstudentwholename").value =
...
function editenrolleerecordsnewstudent(options) {
  /////////////////////// this is for displaying new student ////////////////////////
  async function displaystudenttable() {
    let response = await fetch(
      "/api/superadmin/registrar/admission/displaynewstudent",
      {
        method: "GET",
      }
    );
    ...
    if (myresult[0].id != "invalid") {
      for (var x in myresult) {
        text +=
          "<tr onclick=" +
          '"' +
          "editenrolleerecordsnewstudentdisplaystudentinfo('" +
          myresult[x].id +
          "')" +
          '"' +
          "><td>" +
      ...
      document.getElementById("newstudentresult1").innerHTML = text;
      document.getElementById("newstudentresult2").innerHTML = text2;
    } else {
      document.getElementById("newstudentresult1").innerHTML = text;
      document.getElementById("newstudentresult2").innerHTML = text2;
    }
  ...
  async function approvestudent() {
    ...
    let response = await fetch(
      "/api/superadmin/registrar/admission/approvenewstudent?studentid=" +
        studentid,
      {
        method: "GET",
      }
    ...
    if (myresult[0].id == "success") {
      loadalert();
      document.getElementById("newstudentresult1").innerHTML = "";
      document.getElementById("newstudentresult2").innerHTML = "";
